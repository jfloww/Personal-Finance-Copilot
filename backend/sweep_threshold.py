...
def main(argv: list[str]) -> int:
    ...
    result: dict[str, object] = {
        ...
        "distribution_caveat": (
            "This curve was swept over LLM predictions made with no rule tier "
            "ahead of the model, while the deployed categorise.py pipeline runs an "
            "unfitted rule tier first and sends the model only what it abstains "
            "on, so this curve does not describe the input distribution the "
            "deployed pipeline actually produces; the chosen threshold below is "
            "kept as-is rather than re-picked against it, since it was chosen "
            "once, on the development split, and re-picking now would void that. "
            "An earlier version of this caveat also claimed the sweep used each "
            "row's real account_type. It did not: the benchmark CSV carries no "
            'account_type column, so every scored row used the same "unknown" the '
            "deployed pipeline sends."
        ),
        "development": {
